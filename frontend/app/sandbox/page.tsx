    ...
    <AppShell active="Simulation">
      <div className="page-stack">
        <section className="page-hero">
          <span className="hero-kicker">Stress test</span>
          <div className="hero-title-row">
            <div className="hero-copy">
              <h1 className="text-headline" style={{ color: 'var(--color-on-surface)', marginBottom: 6 }}>Test My Week</h1>
              <p className="text-body">
                Try out bad-case scenarios like getting sick, falling behind, or losing a study day and see how your plan holds up.
              </p>
            </div>
            <div className="hero-meta">
              <span className="meta-pill">{tasks.length} tasks loaded</span>
              <span className="meta-pill">{result ? `RP ${Math.round(result.path_resilience)}%` : 'Ready to simulate'}</span>
            </div>
          </div>
        </section>
        ...
        <div className="content-split" style={{ gap: 24 }}>
          {/* Left: Inject Panel */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div className="glass-active" style={{ borderRadius: 16, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, borderBottom: '1px solid rgba(78,222,163,0.12)', paddingBottom: 16 }}>
                <h3 className="text-label" style={{ color: 'var(--color-on-surface)' }}>ADD A WHAT-IF</h3>
                <span className="material-symbols-outlined" style={{ color: 'var(--color-primary)', fontSize: 20 }}>add_box</span>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label className="text-label" style={{ color: 'var(--color-on-muted)', display: 'block', marginBottom: 8 }}>WHAT HAPPENS?</label>
                  <input className="kronos-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Unexpected Illness" />
                </div>
                <div>
                  <label className="text-label" style={{ color: 'var(--color-on-muted)', display: 'block', marginBottom: 8 }}>HOW MANY HOURS?</label>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <label className="text-label" style={{ color: 'var(--color-on-muted)' }}>HOW SERIOUS IS IT?</label>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="glass" style={{ borderRadius: 16, padding: 24 }}>
              <h3 className="text-label" style={{ color: 'var(--color-on-muted)', marginBottom: 16 }}>ACTIVE SCENARIOS</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {injects.length === 0 && (
                  <p className="text-mono" style={{ color: 'var(--color-on-muted)', fontSize: 12, padding: '8px 0' }}>No custom scenario added yet. Add one on the left or run the stress test with just your current tasks.</p>
                )}
              </div>
            </div>
            ...
            >
              ...
              {running ? 'RUNNING 50 TESTS…' : 'RUN 50 STRESS TESTS'}
            </button>
            ...
            {result && (
              <div className="two-up-grid">
                <div className="glass" style={{ borderRadius: 16, padding: 24 }}>
                  <h4 className="text-label" style={{ color: 'var(--color-on-muted)', marginBottom: 16 }}>CHANCE YOUR PLAN HOLDS UP</h4>
                ...
                <div className="glass" style={{ borderRadius: 16, padding: 24 }}>
                  <h4 className="text-label" style={{ color: 'var(--color-on-muted)', marginBottom: 16 }}>PLAN PRESSURE</h4>
                ...
                <div className="glass" style={{ gridColumn: 'span 2', borderRadius: 16, padding: 24 }}>
                  <h4 className="text-label" style={{ color: 'var(--color-on-muted)', marginBottom: 6 }}>50 TEST RUNS</h4>
